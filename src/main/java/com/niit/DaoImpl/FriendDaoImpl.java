...
public class FriendDaoImpl implements FriendDao
{
	...
	public List<User> suggestedUsers(String email) 
	{
		...
		String queryString="select * from UserDetail where email in (select email from UserDetail where email!=? minus (select toId_email from friend where fromId_email=? union select fromId_email from friend where toId_email=?))";
